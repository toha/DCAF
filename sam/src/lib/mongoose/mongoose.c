...
int ns_socketpair2(sock_t sp[2], int sock_type) {
  ...
  if ((sock = socket(AF_INET, sock_type, 0)) != INVALID_SOCKET &&
      ...
      (sp[1] = (sock_type == SOCK_DGRAM ? sock : accept(sock, &sa.sa, &len))) !=
          INVALID_SOCKET) {
...
int ns_socketpair(sock_t sp[2]) { return ns_socketpair2(sp, SOCK_STREAM); }
...
static int ns_use_ca_cert(SSL_CTX *ctx, const char *cert) {
  ...
  int a = SSL_CTX_load_verify_locations(ctx, cert, NULL) == 1 ? 0 : -2;
  return a;
}
...
void ns_broadcast(struct ns_mgr *mgr, ns_callback_t cb, void *data,
                  size_t len) {
  struct ctl_msg ctl_msg;
  if (mgr->ctl[0] != INVALID_SOCKET && data != NULL &&
    ...
    send(mgr->ctl[0], (char *)&ctl_msg, offsetof(struct ctl_msg, message) + len,
         0);
...
} static_builtin_mime_types[] = {{".html", 5, "text/html"},
                                 {".htm", 4, "text/html"},
                                 {".shtm", 5, "text/html"},
                                 {".shtml", 6, "text/html"},
                                 {".css", 4, "text/css"},
...
void mg_munmap(void *p, size_t size) { munmap(p, size); }
...
static process_id_t start_process(char *interp, const char *cmd,
  ...
  mg_snprintf(cmdline, sizeof(cmdline), "%s%s\"%s\"", interp ? interp : "",
              interp ? " " : "", cmd);
...
static void open_cgi_endpoint(struct connection *conn, const char *prog) {
  ...
  if (start_process(conn->server->config_options[CGI_INTERPRETER], prog,
    ...
    conn->endpoint.nc =
        ns_add_sock(&conn->server->ns_mgr, fds[0], mg_ev_handler, conn);
...
static int is_valid_http_method(const char *s) {
  return !strcmp(s, "GET") || !strcmp(s, "POST") || !strcmp(s, "HEAD") ||
         ...
         !strcmp(s, "OPTIONS") || !strcmp(s, "PROPFIND") ||
...
static void open_file_endpoint(struct connection *conn, const char *path,
  ...
  if (hdr != NULL && (n = parse_range_header(hdr, &r1, &r2)) > 0 && r1 >= 0 &&
    ...
    conn->cl =
        n == 2 ? (r2 > conn->cl ? conn->cl : r2) - r1 + 1 : conn->cl - r1;
...
static int scan_directory(struct connection *conn, const char *dir,
  ...
  *arr = NULL;
  ...
  while ((dp = readdir(dirp)) != NULL) {
    ...
    if (arr_ind >= arr_size) {
      if ((p = (struct dir_entry *)NS_REALLOC(
               *arr, (inc + arr_size) * sizeof(**arr))) != NULL) {
...
static void send_directory_listing(struct connection *conn, const char *dir) {
  ...
  int i, num_entries,
      sort_direction = conn->mg_conn.query_string != NULL &&
                               conn->mg_conn.query_string[1] == 'd'
                           ? 'a'
                           : 'd';
...
static void handle_put(struct connection *conn, const char *path) {
  ...
  } else if ((conn->endpoint.fd = open(
                  path, O_RDWR | O_CREAT | O_TRUNC | O_BINARY, 0644)) < 0) {
...
static void forward_put_data(struct connection *conn) {
  ...
  size_t k =
      conn->cl < (int64_t)io->len ? conn->cl : (int64_t)io->len; // To write
...
int mg_authorize_digest(struct mg_connection *c, FILE *fp) {
  ...
  while (fgets(line, sizeof(line), fp) != NULL) {
    if (sscanf(line, "%[^:]:%[^:]:%s", f_user, f_domain, ha1) == 3 &&
      ...
      return check_password(c->request_method, ha1, uri, nonce, nc, cnonce, qop,
                            resp);
  }
  return MG_FALSE;
}
...
static void do_ssi_include(struct mg_connection *conn, const char *ssi,
  ...
  if ((fp = fopen(path, "rb")) == NULL) {
    mg_printf(conn, "Cannot open SSI #include: [%s]: fopen(%s): %s", tag, path,
              strerror(errno));
  } else {
...
static int is_valid_uri(const char *uri) {
  ...
  return uri[0] == '/' ||
         ...
         mg_strncasecmp(uri, "http", 4) ==
             0 ||                            // Naive check for the absolute URI
...
static void try_parse(struct connection *conn) {
  ...
  if (conn->request_len == 0 &&
    ...
    conn->request_len =
        parse_http_message(conn->request, conn->request_len, &conn->mg_conn);
    if (conn->request_len > 0) {
...
static void close_local_endpoint(struct connection *conn) {
  ...
  conn->cl = conn->num_bytes_recv = conn->request_len = 0;
  conn->ns_conn->flags &=
      ~(NSF_FINISHED_SENDING_DATA | NSF_BUFFER_BUT_DONT_SEND |
        NSF_CLOSE_IMMEDIATELY | MG_HEADERS_SENT | MG_USING_CHUNKED_API);
...
struct mg_connection *mg_next(struct mg_server *s, struct mg_connection *c) {
  struct ns_connection *nc =
      ns_next(&s->ns_mgr, c == NULL ? NULL : MG_CONN_2_CONN(c)->ns_conn);
  ...
  *v = mg_strdup(value);
  ...
  if (ind == LISTENING_PORT) {
    ...
    while ((value = next_option(value, &vec, NULL)) != NULL) {
      struct ns_connection *c =
          ns_bind(&server->ns_mgr, vec.ptr, mg_ev_handler, NULL);
      if (c == NULL) {
