...
#define CBOR_BYTE_FOLLOWS \
  24 /* indicator that the next byte is part of this item */
...
#define CBOR_ADDITIONAL_INFO(stream, offset) \
  (stream->data[offset] & CBOR_INFO_MASK)
...
/**
 * Return the number of bytes that would follow the additional info field @p
 *additional_info
 *
 * @param additional_info Must be in the range [CBOR_UINT8_FOLLOWS,
 *CBOR_UINT64_FOLLOWS]
 */
static unsigned char uint_bytes_follow(unsigned char additional_info) {
  if (additional_info < CBOR_UINT8_FOLLOWS ||
      additional_info > CBOR_UINT64_FOLLOWS) {
    return 0;
  }
...
static size_t encode_int(unsigned char major_type, cbor_stream_t *s,
                         uint64_t val) {
  if (!s) {
    return 0;
  }
...
static size_t encode_bytes(unsigned char major_type, cbor_stream_t *s,
  ...
  memcpy(&(s->data[s->pos]), data,
         length); /* copy byte string into our cbor struct */
  s->pos += length;
  return (bytes_start + length);
}

static size_t decode_bytes(const cbor_stream_t *s, size_t offset, char *out,
                           size_t length) {
  if ((CBOR_TYPE(s, offset) != CBOR_BYTES &&
       CBOR_TYPE(s, offset) != CBOR_TEXT) ||
      !out) {
    return 0;
  }
...
size_t cbor_deserialize_int(const cbor_stream_t *stream, size_t offset,
  ...
  if ((CBOR_TYPE(stream, offset) != CBOR_UINT &&
       CBOR_TYPE(stream, offset) != CBOR_NEGINT) ||
      !val) {
    return 0;
  }
...
size_t cbor_deserialize_array(const cbor_stream_t *s, size_t offset,
                              size_t *array_length) {
...
size_t cbor_deserialize_array_indefinite(const cbor_stream_t *s,
                                         size_t offset) {
...
size_t cbor_deserialize_map(const cbor_stream_t *s, size_t offset,
                            size_t *map_length) {
...
size_t cbor_deserialize_date_time(const cbor_stream_t *stream, size_t offset,
                                  struct tm *val) {
  ...
  size_t read_bytes =
      cbor_deserialize_unicode_string(stream, offset, buffer, sizeof(buffer));
...
size_t cbor_serialize_date_time(cbor_stream_t *stream, struct tm *val) {
  ...
  if (strftime(time_str, sizeof(time_str), format, val) ==
      0) { /* struct tm to string */
    return 0;
  }
...
size_t cbor_deserialize_date_time_epoch(const cbor_stream_t *stream,
                                        size_t offset, time_t *val) {
  if ((CBOR_TYPE(stream, offset) != CBOR_TAG) ||
...
size_t cbor_serialize_date_time_epoch(cbor_stream_t *stream, time_t val) {
  ...
  if (val < 0) {
    return 0; /* we currently don't support negative values for the time_t
                 object */
  }
 ...
 */
static size_t cbor_stream_decode_at(cbor_stream_t *stream, size_t offset,
                                    int indent) {
#define DESERIALIZE_AND_PRINT(type, suffix, format_string)               \
  ...
  switch (CBOR_TYPE(stream, offset)) {
    ...
    case CBOR_BYTES: {
      ...
      size_t read_bytes =
          cbor_deserialize_byte_string(stream, offset, buffer, sizeof(buffer));
      // DEBUG("(byte string, \"%s\")\n", buffer);
      return read_bytes;
    }
    ...
    case CBOR_TEXT: {
      char buffer[CBOR_STREAM_PRINT_BUFFERSIZE];
      size_t read_bytes = cbor_deserialize_unicode_string(
    ...
    case CBOR_ARRAY: {
      ...
      if (is_indefinite) {
        offset += read_bytes =
            cbor_deserialize_array_indefinite(stream, offset);
      ...
      while (is_indefinite ? !cbor_at_break(stream, offset)
                           : i < array_length) {
        size_t inner_read_bytes;
        offset += inner_read_bytes =
            cbor_stream_decode_at(stream, offset, indent + 2);
    ...
    case CBOR_MAP: {
      const bool is_indefinite =
          (stream->data[offset] == (CBOR_MAP | CBOR_VAR_FOLLOWS));
      uint64_t map_length;
      size_t read_bytes;
      ...
      while (is_indefinite ? !cbor_at_break(stream, offset) : i < map_length) {
        ...
        offset += key_read_bytes =
            cbor_stream_decode_at(stream, offset, indent + 1); /* key */
        offset += value_read_bytes =
            cbor_stream_decode_at(stream, offset, indent + 2); /* value */
    ...
    case CBOR_TAG: {
      ...
      switch (tag) {
/* Non-native builds likely don't have support for ctime (hence disable it
 * there)
 * TODO: Better check for availability of ctime functions? */
        ...
        case CBOR_DATETIME_STRING_FOLLOWS: {
          ...
          struct tm timeinfo;
          size_t read_bytes =
              cbor_deserialize_date_time(stream, offset, &timeinfo);
        ...
        case CBOR_DATETIME_EPOCH_FOLLOWS: {
          time_t time;
          size_t read_bytes =
              cbor_deserialize_date_time_epoch(stream, offset, &time);
...
void cbor_stream_decode(cbor_stream_t *stream) {
  ...
  while (offset < stream->pos) {
    ...
    if (read_bytes == 0) {
      // EBUG("Failed to read from stream at offset %d, start byte 0x%02X\n",
