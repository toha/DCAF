...
unsigned char msgtype =
    COAP_MESSAGE_CON; /* usually, requests are sent confirmable */
...
int use_dtls = 1; /* Indicates whether secure
                   * communication was requested by
                   * stating a 'coaps' URI on the
                   * command line. The has only an
                   * effect with libtinydtls. */

sam_coap_response_handler_t sam_coap_handle;
coap_pdu_t *last_received = NULL;
                     ...
                     coap_pdu_t *received, const coap_tid_t id)
{
        ...
        if (!block_opt) {
            ...
            if (COAP_OPT_BLOCK_MORE(block_opt)) {
                ...
                pdu = coap_new_request(
                          ctx, method, NULL); /* first, create bare PDU w/o any option  */
                if (pdu) {
                    /* add URI components from optlist */
                    for (option = optlist; option; option = option->next) {
                        switch (COAP_OPTION_KEY(*(coap_option *)option->data)) {
                    ...
                    coap_add_option(pdu, blktype,
                                    coap_encode_var_bytes(
                                        buf, ((coap_opt_block_num(block_opt) + 1) << 4) |
    ...
    sam_coap_handle(ctx, local_interface, remote, sent, received, id);
}
...
void usage(const char *program, const char *version)
{
    ...
    fprintf(
        ...
        "usage: %s [-A type...] [-t type] [-b [num,]size] [-B seconds] [-e "
        "text]\n"
        ...
        "\t-k file\t\tsecret key\n"
        "\nThese options currently are used with the pre-shared key "
        "ciphersuites\n"
...
#if HAVE_LIBTINYDTLS
        "\tcoap-client -i identity.txt -k key.txt "
        "coaps://example.com/auth?node123\n"
#endif
        ,
...
coap_list_t *new_option_node(unsigned short key, unsigned int length,
                             unsigned char *data)
{
    coap_option *option;
    coap_list_t *node;
    ...
    /* we can pass NULL here as delete function since option is released
     * automatically  */
...
{
    static content_type_t content_types[] = {{0, "plain"},
        {0, "text/plain"},
        {40, "link"},
        {40, "link-format"},
    ...
    while (q && *q) {
        ...
        if (isdigit(*q)) {
            ...
            for (i = 0; content_types[i].media_type &&
                 strncmp(q, content_types[i].media_type,
                         p ? p - q : strlen(q)) != 0;
                 ++i)
                ;
...
int is_default_port(const coap_uri_t *uri)
{
    assert(uri);

    if (uri->port == COAP_DEFAULT_PORT) {
        return uri->scheme.length == strlen(COAP_DEFAULT_SCHEME) &&
               strncasecmp((const char *)uri->scheme.s, COAP_DEFAULT_SCHEME,
                           uri->scheme.length) == 0;
    }

    if (uri->port == COAPS_DEFAULT_PORT) {
        return uri->scheme.length == strlen(COAPS_DEFAULT_SCHEME) &&
               strncasecmp((const char *)uri->scheme.s, COAPS_DEFAULT_SCHEME,
                           uri->scheme.length) == 0;
    }
...
#define BUFSIZE 40
    ...
    if (proxy.length) { /* create Proxy-Uri from argument */
        ...
        while (len > 270) {
            coap_insert(&optlist, new_option_node(COAP_OPTION_PROXY_URI, 270,
                                                  (unsigned char *)arg),
                        order_opts);
            len -= 270;
            arg += 270;
        ...
        coap_insert(&optlist, new_option_node(COAP_OPTION_PROXY_URI, len,
                                              (unsigned char *)arg),
                    order_opts);
...
void set_blocksize()
{
    ...
    if (method != COAP_REQUEST_DELETE) {
        ...
        coap_insert(
            &optlist,
            new_option_node(
                opt, coap_encode_var_bytes(buf, (block.num << 4 | block.szx)), buf),
            order_opts);
    }
}
...
void cmdline_option(char *arg)
{
    ...
    coap_insert(&optlist, new_option_node(num, strlen(arg), (unsigned char *)arg),
                order_opts);
}

extern int check_segment(const unsigned char *s, size_t length);
extern void decode_segment(const unsigned char *seg, size_t length,
                           unsigned char *buf);
...
void handle_read(coap_context_t *ctx, coap_endpoint_t *local)
{
...
ssize_t send_to_peer2(struct coap_context_t *context,
                      const coap_endpoint_t *local_interface,
...
int dtls_application_data2(struct dtls_context_t *ctx, session_t *session,
                           uint8 *data, size_t len)
{
    assert(local_interface);

    return coap_handle_message(dtls_get_app_data(ctx), local_interface,
                               (coap_address_t *)session, (unsigned char *)data,
                               len);
}

int dtls_send_to_peer2(struct dtls_context_t *ctx, session_t *session,
                       uint8 *data, size_t len)
{
    assert(local_interface);

    return coap_network_send(dtls_get_app_data(ctx), local_interface,
...
static dtls_psk_key_t psk = {
    .id = psk_id, .id_length = 0, .key = psk_key, .key_length = 0
};

/* This function is the "key store" for tinyDTLS. It is called to
 * retrieve a key for the given identity within this particular
 * session. */
int get_psk_key2(struct dtls_context_t *ctx, const session_t *session,
...
int connect_remote(dtls_context_t *ctx, const coap_address_t *remote,
...
void close_remote(dtls_context_t *ctx, const coap_address_t *remote)
{
    session_t session;
...
static dtls_handler_t cb = {.write = dtls_send_to_peer2,
                            .read = dtls_application_data2,
                            .event = NULL,
                            .get_psk_key = get_psk_key2,
#ifdef WITH_ECC
                            .get_ecdsa_key = NULL,
...
int coap_client_run(sam_coap_response_handler_t sam_handler,
                    coap_pdu_t **sam_received, int method_p, char *uri_p,
                    char *psk_id_p, size_t psk_id_length_p, char *psk_key_p,
                    size_t psk_key_length_p, char *payload_p,
                    size_t payload_size_p)
{
    ...
    wait_seconds = 2;
    payload.s = payload_p;
    payload.length = payload_size_p;
    psk.id = psk_id_p;
    psk.id_length = psk_id_length_p;
    psk.key = psk_key_p;
    psk.key_length = psk_key_length_p;
    method = method_p;
    last_received = NULL;
    ...
    if (!dtls_context) {
        printf("cannot create DTLS context\n");
    ...
    if (!proxy.length && addrptr &&
        ...
        coap_insert(&optlist, new_option_node(COAP_OPTION_URI_HOST, uri.host.length,
                                              uri.host.s),
                    order_opts);
    }

    // set block option if requested at commandline if (flags & FLAGS_BLOCK)
    set_blocksize();
    ...
    while (!(ready && coap_can_exit(ctx))) {
        ...
        if (result < 0) { /* error */
            ...
            if (obs_wait && obs_wait <= now) {
                printf("clear observation relationship\n");
